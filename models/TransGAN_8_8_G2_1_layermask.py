...
def get_2d_attn_mask(N, w):
    reference = torch.tensor([*range(N ** 2)]).view(N, N)

    mask = np.zeros([1, 1, N ** 2, N ** 2])
    for row in range(N):
        for column in range(N):
            left = max(0, column - w)
            right = min(N, column + w + 1)
            top = max(0, row - w)
            bottom = min(N, row + w + 1)
            masking_indexes = reference[top:bottom, left:right].contiguous().view(-1)
            mask[:, :, row * N + column, masking_indexes] = 1
...
def get_random_attn_mask(N, w):
    mask = np.zeros([1, 1, N ** 2, N ** 2])
    for row in range(N ** 2):
        indices = np.random.choice(range(N ** 2), w ** 2, replace=False)
        mask[:, :, row, indices] = 1
...
class Attention(nn.Module):
    ...
    def forward(self, x, epoch):
        ...
        if type(self.is_mask) == tuple:
            attn = attn.masked_fill(self.mask.to(attn.get_device()) == 0, -1e9)
...
class Generator(nn.Module):
    def __init__(self, args, img_size=224, patch_size=16, in_chans=3, num_classes=10, embed_dim=384, depth=5,
        ...
        print("TransGAN_8_8_G2_1_layermask : mask=(", layer_mask[0].strip(), ", ", layer_mask[1].strip(), ")- mask type=", args.mask_type)
        self.args = args
        self.ch = embed_dim
...
#                         drop=drop_rate, attn_drop=attn_drop_rate, drop_path=0, norm_layer=norm_layer),
                    Block(
                        dim=embed_dim//4, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                        drop=drop_rate, attn_drop=attn_drop_rate, drop_path=0, norm_layer=norm_layer, is_mask=((self.bottom_width*2, 12))),
                    Block(
                        dim=embed_dim//4, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                        drop=drop_rate, attn_drop=attn_drop_rate, drop_path=0, norm_layer=norm_layer, is_mask=((self.bottom_width*2, 12)))
                 ]
...
#                         drop=drop_rate, attn_drop=attn_drop_rate, drop_path=0, norm_layer=norm_layer),
                    Block(
                        dim=embed_dim//16, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                        drop=drop_rate, attn_drop=attn_drop_rate, drop_path=0, norm_layer=norm_layer, is_mask=((self.bottom_width*4, 12))),
                    Block(
                        dim=embed_dim//16, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                        drop=drop_rate, attn_drop=attn_drop_rate, drop_path=0, norm_layer=norm_layer, is_mask=((self.bottom_width*4, 12)))
                 ]
